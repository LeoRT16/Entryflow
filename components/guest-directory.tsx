...
type DeliveryStatus = "Pendiente de envío" | "Enviada" | "Reenviada" | "Vista" | "Fallida";
type AdmissionStatus = "Pendiente" | "Ingresó" | "Anulada" | "Bloqueada";
type ReservationStatus = "Confirmada" | "Pendientes de pago" | "Pago parcial" | "Cancelada";
type AdmissionFilter = "Todos" | "Pendientes" | "Ingresaron" | "Anulados" | "Bloqueados";
type ReservationFilter = "Todas" | "Confirmadas" | "Pendientes de pago" | "Pago parcial" | "Canceladas";
...
type IncidentSeverity = "info" | "warning" | "critical" | "resolved";
...
const eventOptions: EventOption[] = [
  { name: "Noche Carlota", status: "En curso" },
  { name: "Viernes Retro", status: "Próximo" },
  { name: "Fiesta Blanca", status: "Próximo" },
];
...
const admissionFilters: AdmissionFilter[] = [
  "Todos",
  "Pendientes",
  "Ingresaron",
  "Anulados",
...
const deliveryFilters: Array<"Todos" | DeliveryStatus> = [
  "Todos",
  "Pendiente de envío",
  "Enviada",
  "Reenviada",
...
const reservationFilters: ReservationFilter[] = [
  "Todas",
  "Confirmadas",
  "Pendientes de pago",
  "Pago parcial",
...
function admissionFilterToStatus(filter: AdmissionFilter): AdmissionStatus | null {
...
function reservationFilterToStatus(filter: ReservationFilter): ReservationStatus | null {
...
function buildTimeline(guest: GuestRecord): TimelineEntry[] {
...
function buildOperationalNotes(guest: GuestRecord): OperationalNote[] {
...
function getGuestIncidents(guest: GuestRecord) {
  return guestIncidentLibrary[guest.id]?.incidents ?? [];
}

function getGuestAuditRows(guest: GuestRecord) {
  return guestIncidentLibrary[guest.id]?.auditRows ?? [];
}

function getIncidentVariant(severity: IncidentSeverity) {
...
function getIncidentToneClass(severity: IncidentSeverity) {
  if (severity === "critical") {
...
export default function GuestDirectory() {
  ...
  const [selectedEvent, setSelectedEvent] = useState(eventOptions[0].name);
  ...
  const activeEvent = useMemo(
    () => eventOptions.find((item) => item.name === selectedEvent) ?? eventOptions[0],
    [selectedEvent],
  );

  const selectedEventStats = eventStats[activeEvent.name];

  const visibleGuests = useMemo(() => {
    const baseGuests = guestRecords.filter((guest) => guest.eventName === activeEvent.name);
  ...
  }, [activeEvent.name, admissionFilter, deliveryFilter, quickFilterKeys, reservationFilter, searchQuery]);

  const attentionGuests = useMemo(
    () => guestRecords.filter((guest) => guest.eventName === activeEvent.name && guest.attention),
    [activeEvent.name],
  );

  const selectedGuest = useMemo(
    () => guestRecords.find((guest) => guest.id === selectedGuestId) ?? null,
    [selectedGuestId],
  );
      ...
      <section className="grid gap-4 rounded-[2rem] border border-white/10 bg-white/[0.03] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.22)] xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-slate-950/40 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <StatusBadge variant="success">{eventStats[selectedEvent].checkedIn} ingresados</StatusBadge>
            <StatusBadge variant="warning">{eventStats[selectedEvent].pending} pendientes</StatusBadge>
            <StatusBadge variant="danger">{eventStats[selectedEvent].attention} atención</StatusBadge>
          </div>
        </div>
      </section>
