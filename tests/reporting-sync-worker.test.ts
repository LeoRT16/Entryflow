...
test("same hash skips write only for same destination identity", async () => { let wrote = false; const { calls, deps } = makeDeps({ transport: { ...transport, clear: async () => { wrote = true; } }, repository: { claim: async () => [work], completeSuccess: async () => { calls.push("success"); }, completeFailure: async () => { calls.push("failure"); }, lastSuccessfulHash: async () => ({ datasetHash: "bee150417499119c40d7171abae18ca7031f8ebf2947da4b566972242dddc046", spreadsheetId: "s1", sheetSchemaVersion: 1 }) } }); const result = await processReportingSyncBatch(deps, "worker-1"); assert.equal(result.skipped, 1); assert.equal(wrote, false); assert.deepEqual(calls, ["success"]); });
