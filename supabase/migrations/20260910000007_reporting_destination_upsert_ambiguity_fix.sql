...
begin
  ...
  select (rd.spreadsheet_id is distinct from trim(p_spreadsheet_id)) into changed
  from public.reporting_destinations as rd
  where rd.event_id=p_event_id and rd.provider='google_sheets' and rd.deleted_at is null;
  insert into public.reporting_destinations(organization_id,event_id,provider,spreadsheet_id,enabled,sheet_schema_version)
    select e.organization_id,p_event_id,'google_sheets',trim(p_spreadsheet_id),coalesce(p_enabled,true),1 from public.events as e where e.id=p_event_id
  ...
  select rd.* into d from public.reporting_destinations as rd where rd.event_id=p_event_id and rd.provider='google_sheets' and rd.deleted_at is null;
