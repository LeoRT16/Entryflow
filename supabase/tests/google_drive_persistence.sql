...
begin
 ...
 update public.reporting_drive_integrations set organization_drive_folder_id=null,reserved_organization_drive_folder_id=null where id=ia;
 if public.reserve_drive_organization_folder_id(ia,'candidate-a')<>'candidate-a' or public.reserve_drive_organization_folder_id(ia,'candidate-b')<>'candidate-a' then raise exception 'root reservation first-writer failed'; end if;
 if not public.confirm_drive_organization_folder(ia,'candidate-a') or public.confirm_drive_organization_folder(ia,'candidate-b') then raise exception 'root confirmation conflict failed'; end if;
 ...
 l:=(select id from public.event_drive_locations where event_id=ea); update public.event_drive_locations set event_drive_folder_id=null,final_reports_folder_id=null,reserved_event_drive_folder_id=null,reserved_final_reports_folder_id=null where id=l;
 if public.reserve_drive_event_folder_id(l,'event-candidate-a',1)<>'event-candidate-a' or public.reserve_drive_event_folder_id(l,'event-candidate-b',1)<>'event-candidate-a' then raise exception 'event reservation failed'; end if;
 if public.reserve_drive_reports_folder_id(l,'reports-candidate-a',1)<>'reports-candidate-a' or public.reserve_drive_reports_folder_id(l,'reports-candidate-b',1)<>'reports-candidate-a' then raise exception 'reports reservation failed'; end if;
 if not public.confirm_drive_event_folder(l,'event-candidate-a') or public.confirm_drive_event_folder(l,'event-candidate-b') then raise exception 'event confirmation conflict failed'; end if;
 if not public.confirm_drive_reports_folder(l,'reports-candidate-a') or public.confirm_drive_reports_folder(l,'reports-candidate-b') then raise exception 'reports confirmation conflict failed'; end if;
 ...
 update public.event_drive_locations set reserved_event_drive_folder_id='event-reclaim-X' where event_id=ea;
 if (select reserved_event_drive_folder_id from public.event_drive_locations where event_id=ea)<>'event-reclaim-X' then raise exception 'reservation setup failed'; end if;
 ...
 if (select reserved_event_drive_folder_id from public.event_drive_locations where event_id=ea)<>'event-reclaim-X' then raise exception 'reservation lost on reclaim'; end if;
 ...
 update public.event_drive_locations set revision=c.target_revision where event_id=ea; if not public.complete_drive_provisioning_job(c.outbox_id,new,c.target_revision,'event-reclaim-X','reports-reclaim-X') then raise exception 'current completion rejected'; end if;
...
select pass('google_drive_persistence comprehensive checks');
select * from finish();
rollback;
