...
export async function PATCH(
  ...
  const current = await repositories.enrollments.getById(enrollmentId);

  if (!current || current.organizationId !== scope.event.organizationId || current.eventId !== scope.event.id) {
    return NextResponse.json(
      { ok: false, error: { code: "enrollment_not_found", message: "No pudimos resolver la inscripción." } },
      { status: 404 },
    );
  }

  const enrollment = await repositories.enrollments.update(enrollmentId, {
    name,
    ...
    metadata: mergeAccreditationParticipantMetadata(current.metadata, {
...
export async function DELETE(
  ...
  const enrollment = await repositories.enrollments.cancel(enrollmentId);
