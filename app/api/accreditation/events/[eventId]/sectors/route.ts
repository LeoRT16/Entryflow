...
async function handlePOST(
  ...
  const code = getRequestString(body.code);

  if (!name || !code) {
    return NextResponse.json(
      { ok: false, error: { code: "missing_fields", message: "El sector necesita nombre y código." } },
      { status: 400 },
    );
  }
  ...
  try {
    const sector = await repositories.sectors.create({
      organizationId: scope.event.organizationId,
      eventId: scope.event.id,
      name,
      code,
      description: getRequestString(body.description) || null,
      status: body.status,
      capacity: getRequestNumber(body.capacity) ?? null,
      sortOrder: getRequestNumber(body.sortOrder) ?? 0,
